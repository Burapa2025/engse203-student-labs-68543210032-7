...
| เวลา | งาน | Prompt ที่ถาม | AI ตอบว่าอะไร | ฉันทำอะไรต่อ |
|---|---|---|---|---|
| 14:11 | B1 | พอลบการ์ดแล้วทำไมตัวเลขสรุปใน useMemo ไม่ยอมลดลงตามข้อมูลจริง | ให้ตรวจ dependency array ของ useMemo สำหรับ summary | แก้เปลี่ยนจาก [] เป็น [requests] เพื่อให้ตัวเลขแผงสรุปคำนวณใหม่เมื่อลบ |
| 15:31 | B2 | CP-B2.1 สอนวิธีสร้างช่อง input ค้นหาพร้อมผูก state เก็บบันทึกค่าใน React หน่อย | ให้สร้าง state searchText และเพิ่มแท็ก input พร้อม onChange | สร้าง state searchText และผูก event เข้ากับ input element เหนือรายการคำร้อง |
| 15:38 | B2 | CP-B2.2 เขียน filter ให้ค้นหาจาก requestType หรือ location โดยไม่สนตัวพิมพ์เล็กใหญ่ | แนะนำให้ใช้ .toLowerCase() และ .includes() ตรวจสอบทั้งสอง field | นำไปปรับใช้ใน filteredRequests และทดสอบพิมพ์ "ซ่อม" แสดงผล 2 การ์ดตรงโจทย์ |
| 15:45 | B2 | CP-B2.3 เขียน logic ใน filter อย่างไรให้กรองได้ทั้งสถานะ และค้นหาจากประเภท/สถานที่พร้อมกัน | ให้โครงสร้าง .filter() รวมเงื่อนไขสถานะและคำค้นหา (matchStatus && matchSearch) | นำไปปรับใช้ใน DashboardPage.jsx และทดสอบเลือก "เสร็จสิ้น" + พิมพ์ "ซ่อม" เหลือ REQ-104 |
